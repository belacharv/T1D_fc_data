...
plotting_axis <- function(population,data,name,i,l) 
{
  ...
  ggplot(aes(x = group_var, y = population), data=data) +
    ...
    guides(fill = FALSE)+
    scale_fill_manual(values = col0)
  Filename <- paste(c(l,"_plotaxis",i,".svg"),collapse = "")
  ggsave(filename=Filename,width=9,height = 12,units = "cm")
}
...
for (j in 1:n_clusters_p) {
  cluster <- clusters_list_of_tables2[[j]] #value
  name <- paste("Cluster_",j,sep="")
  plotting(cluster$value,cluster,name,j,20)
}
##########################################################################
################################################################################
...
merged6 <- cutree(hc, k = 6)

# Map back to original clusters
# Create a mapping from old cluster IDs to new merged66 cluster IDs
...
condor$clustering$phenograph_pca_norm_k_60_merged6$Phenograph <- cluster_mapping$new_cluster[
  match(original_clusters, cluster_mapping$old_cluster)
]

cat("merged6 clusters:", length(unique(condor$clustering$phenograph_pca_norm_k_60_merged6)), "\n")


# After creating the merged clusters, format it as a data frame
condor$clustering$phenograph_pca_norm_k_60_merged6 <- data.frame(
  Phenograph = cluster_mapping$new_cluster[
    match(original_clusters, cluster_mapping$old_cluster)
  ],
  Description = "merged_k6"
)

# Set rownames to match original
rownames(condor$clustering$phenograph_pca_norm_k_60_merged6) <- 
...
p <- plot_dim_red(fcd = condor,
                  ...
                  reduction_method = "umap",
                  reduction_slot = "pca_norm",
                  cluster_slot = "phenograph_pca_norm_k_60_merged6",
                  param = "Phenograph",
                  title = "UMAP Phenograph Merged (k=5)")
p$data[["poi"]] <- factor(p$data[["poi"]])
condor$clustering$phenograph_pca_norm_k_60_merged6
p + 
  scale_color_manual(values = c("#D32248", "#D35422","#D3AD22", "#22D3AD", "#2248D3","#000"))+
...
p + 
    scale_color_manual(values = c("#979292","#979292","#979292", "#22D3AD", "#979292","#979292"))+
    #scale_color_manual(values = c("#2BC7F3","#2B64F3","#2BF3BA"))+
    guides(color = guide_legend(override.aes = list(size = 5)))
  ggsave(filename = "plot445.svg",width=12,height = 12,units = "cm")
expr_data <- condor$expr$orig
cluster_data_pheno_merged6 <- condor$clustering$phenograph_pca_norm_k_60_merged6$Phenograph
#create_heatmap(expr_data,cluster_data_pheno_merged6,"pheno_merged6")
...
for (i in 1:6) {
  ...
  newCluster <- plots[[name]][["data"]]
  clusters_list_of_tables2[[i]] <- newCluster
  filename <- paste("Pheno_merged6_",name,".csv",sep="")
  write.csv(newCluster,file.path(save_clusters,filename))
}

for (j in 1:6) {
  cluster <- clusters_list_of_tables2[[j]] #value
  name <- paste("Cluster_merged6_",j,sep="")
...
for (j in 1:6) {
  cluster <- clusters_list_of_tables2[[j]] #value
  name <- paste("Cluster_merged6_",j,sep="")
  plotting_axis(cluster$value,cluster,name,j,4)
}
...
generate_heatmaps("phenograph_pca_norm_k_60","_og_")
generate_heatmaps("phenograph_pca_norm_k_60_merged6","_merged6_")
